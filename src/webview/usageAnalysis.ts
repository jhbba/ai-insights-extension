...
import { AggregatedMetrics, RepositoryHygieneReport, FileStatus } from '../types';
...
function buildCostIntelligenceTab(m: AggregatedMetrics): string {
  const b = m.budget;
  const roi = m.roi;
  ...
  <!-- ROI & Efficiency ───────────────────────────────────── -->
  <div class="section">
    <h2>📈 ROI &amp; Efficiency (This Month)</h2>
...
function getHtml(m: AggregatedMetrics, reports: RepositoryHygieneReport[]): string {
...
export class UsageAnalysisProvider {
  ...
  static createPanel(
    ...
    if (UsageAnalysisProvider.currentPanel) {
      UsageAnalysisProvider.currentPanel.webview.html = getHtml(metrics, reports);
    ...
    panel.webview.html = getHtml(metrics, reports);
